...
def render_cards(cards):
    ...
    for c in cards:
        ...
        if c["photos"]:
            ...
            lines.append('    <div class="atlas-media atlas-empty" data-kingdom="%s" role="img" aria-label="%s, photograph to come">'
                         % (c["kingdom"], esc(c["english"])))
            lines.append('      <svg class="atlas-picto"><use href="#pic-%s"/></svg><span class="atlas-soon">Photograph to come &middot; <span lang="hi">फ़ोटो जल्द</span></span>' % c["picto"])
            lines.append("    </div>")
